...
export const MarketChatAgentDrawer: React.FC<MarketChatAgentDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[420px] bg-zinc-950/98 backdrop-blur-lg border-l border-zinc-800 shadow-2xl flex flex-col justify-between">
      
      {/* Header */}
      <div className="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50">
        <div className="flex items-center space-x-2.5">
          <div className="w-7 h-7 rounded bg-zinc-800 border border-zinc-700 flex items-center justify-center text-emerald-400">
            <Bot className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-xs font-semibold text-zinc-100 flex items-center space-x-1.5">
              <span>Benchia Copilot</span>
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
            </h3>
            <p className="text-[10px] text-zinc-400 truncate max-w-[220px] font-mono">
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3.5 text-xs">
          ...
          >
            ...
            {msg.suggestions && msg.suggestions.length > 0 && (
              <div className="mt-2 space-y-1 w-full max-w-[88%]">
                <span className="text-[10px] text-zinc-500 font-mono block">Preguntas sugeridas:</span>
                {msg.suggestions.map((sug, i) => (
                  <button
                    key={i}
                    onClick={() => handleSendMessage(sug)}
                    className="w-full text-left text-[11px] px-2.5 py-1.5 rounded bg-zinc-900 hover:bg-zinc-850 text-zinc-300 border border-zinc-800 transition-colors flex items-center justify-between cursor-pointer"
                  >
                    <span className="truncate">{sug}</span>
                    <ChevronRight className="w-3 h-3 shrink-0 ml-1 text-zinc-500" />
                  </button>
                ))}
              </div>
            )}
        ...
        {isLoading && (
          <div className="flex items-center space-x-2 p-2.5 bg-zinc-900 rounded border border-zinc-800 text-zinc-400 text-xs">
      ...
      <div className="p-3 border-t border-zinc-800 bg-zinc-900/40">
        ...
        >
          <input
            ...
            disabled={isLoading}
            className="w-full pl-3 pr-10 py-2 rounded bg-zinc-900 border border-zinc-700 text-zinc-200 placeholder-zinc-500 text-xs focus:outline-none focus:border-zinc-500 transition-colors disabled:opacity-60"
          />
          <button
            type="submit"
